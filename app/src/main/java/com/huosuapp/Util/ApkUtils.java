...
public class ApkUtils {
    ...
    public static boolean install(DownloadInfo downloadInfo) {
        ...
        String packageNameByApkFile = ApkUtils.getPackageNameByApkFile(MyApplication.getContext(), downloadInfo.getFileSavePath());
        boolean installApp = ApkUtils.isInstallApp(MyApplication.getContext(), packageNameByApkFile);
        if (installApp && downloadInfo.getIsInstallSuccess() == 1) {//从这个盒子安装的
            downloadInfo.setPackageName(packageNameByApkFile);
            try {
                boolean isOpen = ApkUtils.openAppByPackageName(MyApplication.getContext(), downloadInfo.getPackageName());
                if (!isOpen) {
